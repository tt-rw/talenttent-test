...
**Laatste update:** 26-09-2026 (TT-336) — **TT-336 gebouwd en getest: wie 16
of ouder is, bevestigt zijn e-mailadres na "Profiel aanmaken". Wacht op
Ronald (SQL-script, twee Edge Functions, repo-bestanden) en daarna laag 2.**

**Stand van de P0's.** Vier P0-bouwtickets staan open: TT-65 · TT-325 · TT-329 ·
TT-336. TT-336 en TT-344 zijn gebouwd en wachten op laag 2; TT-336 wacht eerst
op de invoer in Supabase. Daarnaast de controle TT-323.

**Besluiten Ronald, 26-09-2026 (TT-336):**
- **1a:** een band oprichten kan pas na de klik in de mail. Wie een band
  opricht, is via die band te vinden. *(Was: advies Claude.)*
- **2a:** een verkeerd getypt e-mailadres is aan te passen op het blok "Nog
  één stap". De mail gaat dan meteen naar het nieuwe e-mailadres. Supabase
  stuurt daarbij zelf geen mail.
- **Woordkeus:** altijd "e-mailadres", nooit los "adres". Ronald: "noem het dan
  e-mailadres. hier is het ook misgegaan in de emailteksten." Zie TT-350.

**Wat er gebouwd is.**
1. **Database** (`_niet-uploaden-sql-editor-26-09-2026-TT-336.sql`): de kolommen
   `musicians.email_bevestigd_op` en `musicians.wacht_op_bevestiging`, de tabel
   `email_bevestiging` en drie triggers. Drukt iemand van 16 of ouder op
   "Profiel aanmaken" zonder bevestigd e-mailadres, dan blijft het profiel
   onzichtbaar en gaat `wacht_op_bevestiging` aan. De app kan de twee kolommen
   niet zelf zetten. Zonder bevestigd e-mailadres weigert de database een
   bericht en een nieuwe band — ook voor wie de wizard halverwege liet liggen.
   Een profiel dat online staat, telt als bevestigd; het script wijzigt geen
   bestaande rij.
2. **Edge Function `email-bevestigen`** (nieuw): `start`, `e-mailadres` en
   `bevestig`. Mail 9, opmaak A. Minstens een minuut tussen twee mails, hooguit
   vijf per etmaal. Oude links blijven werken tot het e-mailadres wordt
   aangepast.
3. **Edge Function `welkom`**: de regel "Niet zelf aangemeld?" alleen nog
   zonder bevestiging, dus bij 13 t/m 15.
4. **App:** na "Profiel aanmaken" gaat een 16-plusser naar Mijn Profiel, met
   bovenaan het blok "Nog één stap: bevestig je e-mailadres" (knop "Mail
   opnieuw sturen" en "E-mailadres klopt niet? Pas het aan"). De knop in de
   mail opent `#bevestig/<code>` op elk toestel; dezelfde pagina als die van de
   ouder, dus geen zestiende view. Berichten en "Band aanmaken" zeggen eerst
   in een gewone zin wat er nog moet.

**Rechtgezet (TT-336-rij, "Onderweg gezien"):** daar stond als **Aanname** dat
"Profiel aanmaken" bij 16+ eindigt op "Profiel bijgewerkt!" en de voortgang
niet wist. **Geverifieerd:** blok 43 zakt tegen de oude code met precies dat
resultaat. Het is in deze bouw opgelost: een nieuwe registratie eindigt nu op
"Profiel aangemaakt!" of op "Nog één stap", en de voortgang is gewist.

**Testset:** blok 43, achttien controles plus één op paginafouten. Tegen de
oude code zakken ze. **622 van 622 geslaagd.** Monitor: 21 van 21 na het
bijwerken van `CHECKSUMS.txt`. Het SQL-script is twee keer gedraaid op een
lokale PostgreSQL 16 (herhaalbaar) en daar met acht gevallen doorgelopen:
16+ wacht, de app kan zichzelf niet bevestigen, geen bericht of band,
bevestigen zet online en start de welkomstmail, 13 t/m 15 gaat direct online,
wie 16 wordt blijft online, een onbekende leeftijd telt als 16+.

**Volgorde voor Ronald:** eerst het SQL-script, dan de Edge Function
`email-bevestigen` (nieuw, Verify JWT uit), dan `welkom`. Pas daarna naar
beide repo's, hoofdmap: `core.js`, `wizard.js`, `ouder.js`, `bands.js`,
`messages.js`, `index.html`, `actielijst.md`, `CHECKSUMS.txt`; en
`tt_tests.py` naar `tests/`.
**Laag 2 daarna, door Claude:** een nieuw 16+-proefaccount; vóór de klik niet
vindbaar, geen bericht, geen band; e-mailadres aanpassen; na de klik vindbaar,
welkomstmail zonder "Niet zelf aangemeld?".

**Gewijzigd:** `core.js`, `wizard.js`, `ouder.js`, `bands.js`, `messages.js`,
`index.html` (het blok op Mijn Profiel en vijf `?v=`), `tests/tt_tests.py`,
`actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 26-09-2026 (TT-344) — **TT-344 gebouwd: de link uit
"Kies een nieuw wachtwoord" blijft op het scherm "Nieuw wachtwoord". Het
ontwerp van TT-336 ligt vast; bouwen in een eigen sessie.**

**Stand van de P0's.** Vier P0-bouwtickets staan open: TT-65 · TT-325 · TT-329 ·
TT-336. TT-344 is gebouwd en wacht op laag 2. Daarnaast de controle TT-323.
**Besluit Ronald, 26-09-2026:** TT-344 hoort bij de P0-punten ("eerst de P0
punten"; op de vraag of TT-344 erbij hoort: "Ja, eerst TT-344").

**TT-344 — de oorzaak, geverifieerd.** Op talenttent.org gemeten (zie hieronder,
TT-323 vervolg): de herstellink eindigde op Mijn Profiel. In de testset is de
volgorde nagebootst zoals supabase-js die doet: het #-deel verdwijnt vóór
`getSession()` antwoordt, `PASSWORD_RECOVERY` komt daarna (setTimeout 0), en de
database antwoordt trager. De oude code eindigt dan op `view-myprofile`, met
het gebruikersnaamscherm erover als de gebruikersnaam ontbreekt. Gelijk aan de
meting op de echte site.

**De fix, in `core.js`.**
1. `herstelLinkBijStart` legt bij het laden vast of de link `type=recovery`
   draagt. Dat gebeurt vóór de client bestaat; supabase-js wist het #-deel
   later.
2. Die link telt mee in `opstartHerstelt`. `onUserLoggedIn()` stuurt dan niet
   meer door naar Mijn Profiel.
3. `appInit()` toont meteen `view-reset`, zonder extra stap in de
   geschiedenis. De melding `PASSWORD_RECOVERY` opent het scherm niet nog een
   keer.
4. Op "Nieuw wachtwoord" komt het gebruikersnaamscherm (TT-38) niet
   tevoorschijn, net als op de goedkeuringspagina (TT-337).

**Testset:** blok 42, drie controles. Tegen de oude code zakken ze alle drie
(`view-myprofile`). **603 van 603 geslaagd.** Monitor: alleen D3 (checksums)
zakte vóór het bijwerken van `CHECKSUMS.txt`.

**TT-336 — besluiten Ronald, 26-09-2026:**
- Bevestigen gebeurt aan het eind: na "Profiel aanmaken".
- Het wachtwoord: Ronald vroeg om de gangbare werkwijze ("als het onlogisch
  is, dan moeten we het niet willen"). Claude stelde voor: e-mail en
  wachtwoord in stap 1 zoals nu, en het profiel pas zichtbaar na de klik.
  Ronald koos daarbij **"a"**: vóór de klik mag iemand rondkijken en zoeken,
  maar niet gevonden worden en geen berichten sturen.
- 13 t/m 15 blijft zoals het is (route A).
Het volledige ontwerp staat in de rij van TT-336.

**Volgorde voor Ronald:** `core.js`, `index.html`, `actielijst.md` en
`CHECKSUMS.txt` naar beide repo's, hoofdmap; `tt_tests.py` naar `tests/`.
Daarna laag 2 door Claude: een nieuwe herstelmail, de link openen, een nieuw
wachtwoord kiezen.

**Gewijzigd:** `core.js`, `index.html` (alleen `?v=` van `core.js`),
`tests/tt_tests.py`, `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 26-09-2026 (TT-323, vervolg) — **TT-323 doorgelopen: alle
acht mails ontvangen. TT-337 laag 2 geslaagd. Eén nieuwe bevinding met advies
P0: TT-344, "Wachtwoord vergeten" werkt niet. Geen codewijziging.**

**Stand van de P0's.** Vier P0-bouwtickets staan open: TT-65 · TT-325 · TT-329 ·
TT-336. Daarnaast TT-344 (advies P0, door Ronald te bevestigen) en de controle
TT-323. Afgehandeld: TT-295, TT-62 (deel 2) en TT-337.

**Werkwijze.** Elke mail heette bij zijn onderwerpregel, en Ronald kreeg één
stap tegelijk (les uit de eerste ronde). Claude deed de kant van de app in het
scherm rechts, op 375px breed. Ronald keek de mails na en draaide de twee
SQL-regels. Wachtwoorden typte Ronald zelf.

**Stand per mail** (onderwerpregel zoals in de inbox; map in Proton):

| Onderwerpregel | Aan | Map | Uitslag |
|---|---|---|---|
| Uke (14) vraagt je toestemming voor Talent Tent | talenttent+ouder@proton.me | Sociaal | aangekomen; afzender "Talent Tent"; oogt goed op de desktop |
| Uke wacht nog op je toestemming | talenttent+ouder@proton.me | Sociaal | aangekomen; de knop werkt (TT-330) |
| Je hebt toestemming — maak je profiel af | talenttent+kind2@proton.me | Inbox | aangekomen; oogt goed; knop niet getoetst (TT-331) |
| Je bent binnen, Uke! Welkom in de Tent | talenttent+kind2@proton.me | Inbox | aangekomen; "deze link" en de voet kloppen (TT-72, TT-334) |
| begint met "[13 t/m 15] Melding:" | privacy@talenttent.org | Inbox | aangekomen |
| Bevestig je nieuwe e-mailadres | talenttent+wijzig2@proton.me | Inbox | aangekomen; de knop wijzigt het adres en opent het profiel |
| Kies een nieuw wachtwoord | talenttent+wijzig2@proton.me | Inbox | aangekomen; **de knop komt uit op Mijn Profiel, niet op het scherm "nieuw wachtwoord"** → TT-344 |
| Je hebt nieuwe berichten en matches | talenttent+kind@proton.me | Sociaal | aangekomen; het bericht van Uke en Uke als nieuwe muzikant staan erin; beide knoppen werken (TT-295, TT-62 deel 2) |

"[2]" voor een onderwerp in Proton is geen deel van de mail: Proton zet twee
mails met hetzelfde onderwerp in één gesprek. Geverifieerd: het onderwerp in de
code bevat geen cijfer.

**TT-337 — laag 2 geslaagd.** Ronald logde in zijn eigen browser in op
talenttent.org en tikte op de knop in de eerste mail aan de ouder. Na tien
tellen stond de goedkeuringspagina er nog, met vinkje en knoppen (melding
Ronald).

**TT-330 — geslaagd.** De link uit de eerste mail werkte nog nadat de
herinnering een nieuwe code had gemaakt. Met de link uit de herinnering gaf
Ronald toestemming. **TT-332:** het eindscherm sluit af met "Je kunt dit
venster nu sluiten." (schermafdruk Ronald). **TT-333:** het wachtscherm van het
kind zegt "Je aanvraag is verstuurd" (schermafdruk Claude).

**DKIM, SPF en DMARC — geverifieerd** uit de kopregels van de digest:
`dkim=pass (2048-bit key) header.d=talenttent.org`, `spf=pass
smtp.mailfrom=talenttent.org`, `dmarc=pass (p=none)`. Daarmee is "DKIM onbekend"
uit de eerste ronde afgerond; bij TT-298 is het rechtgezet. Dat drie mails in
Sociaal belanden, komt dus niet door de authenticatie (TT-345).

**TT-344 — "Wachtwoord vergeten" werkt niet. Advies P0.** Geverifieerd: Ronald
kopieerde een verse link uit "Kies een nieuw wachtwoord". Claude opende die
uitgelogd in het scherm rechts. Na vier seconden stond de app op Mijn Profiel,
ingelogd; het scherm "nieuw wachtwoord" was niet te zien. Ronald zag in zijn
eigen browser twee keer hetzelfde. Zie de rij van TT-344 voor de oorzaak.

**Teksten in de mails.** Ronald: "veel teksten uit de emails zijn helaas nog
niet goed. de excel werkt niet goed volgens mij." Geverifieerd: de teksten in
de functies zijn gelijk aan de kolom "Nieuw" van
`_niet-uploaden-TT-325-mailteksten-25-09-2026.xlsx` en van
`_niet-uploaden-welkomstmail-voorstel-26-09-2026.xlsx`. Er zijn drie
uitzonderingen, en die komen van latere besluiten: "Tik dan op deze link"
(TT-334), "toestel en browser" (25-09-2026, vervolg 14) en het onderwerp van de
meldmail, dat de code opbouwt. Het Excel-bestand is niet aan de mails
gekoppeld; dat wordt TT-329. **Wacht op Ronald:** per mail wat er anders moet.
Dit hoort bij TT-325.

**Voorstel van Claude:** alle bevindingen uit TT-323 in één ronde oplossen,
zoals op 26-09-2026 afgesproken, en daarna de gewijzigde mails opnieuw
ontvangen.

**Nieuw:** TT-344 (advies P0), TT-345 (advies P1), TT-346 en TT-347 (advies
P2), TT-348 en TT-349 (advies P3). Zie de rijen.

**Rechtgezet:** in de P0-tabel begon de rij van TT-325 met "Nieuw". Het ticket
was toen al gebouwd (25-09-2026, vervolg 14) en door Ronald ingevoerd (vervolg
15). Waaruit blijkt: de twee updates van die dag hieronder. Bij TT-298 stond
"Nog niet gecontroleerd: of SPF en DKIM voor `talenttent.org` goed staan"; beide
slagen, zie hierboven.

**Proefgegevens die nu bestaan:**
- Account "Uke" (gebruikersnaam `ukeproef323`, 14 jaar, Ukulele, postcode
  2511). Inlogadres nu `talenttent+wijzig2@proton.me`, was
  `talenttent+kind2@proton.me`. Aanvraag
  `39dac6fe-4d3b-4f98-b5b5-688ac2ac5561`, goedgekeurd; `aangemaakt_op` staat
  acht dagen terug.
- Account "Proef" (`proeftt323`, `talenttent+kind@proton.me`): een bewaarde
  zoekopdracht (Ukulele, 10 km, Den Haag) en één bericht van Uke.
- Eén proefmelding van Uke over Proef in `musician_reports`, reden "Iets
  anders", toelichting "Proefmelding TT-323, geen echte melding."
- Het scherm rechts is ingelogd als Uke, via de herstellink.

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 26-09-2026 (TT-341, stap 3, vervolg) — **De tegel Thema
...
- `index.html`: een klein blok bovenin, vóór `styles.css`, past de keuze toe
  ...
  of zwart. In Instellingen staat bovenaan de tegel **"Thema"** met
...
**TT-336.** Besluit Ronald, 26-09-2026: P0. Was advies P0.

**TT-295 — besluiten Ronald, 26-09-2026:** een bewaarde zoekopdracht (niet het
oude veld terug); één per muzikant, een nieuwe vervangt de oude; alle filters
behalve de naam; alleen het tabblad Muzikant. **Setlist: "voor later"** →
TT-339.

**Gemeten vóór het bouwen (browserpane, SQL Editor, query gedraaid door
Ronald):** `tt_digest_new_musicians` deed een inner join op `musician_wanted`.
`tt_search_musicians` heeft geen filter op blokkades; dat doet de app zelf.
`tt_digest_new_bands` rekent de afstand uit maar filtert er niet op → TT-338.

**Gebouwd.** Onder het resultaat op het tabblad Muzikant staat een blok "Mail
me als er iemand bijkomt", met de zoekopdracht in woorden en de knop "Bewaar
deze zoekopdracht". Na bewaren: "Je krijgt een mail als er iemand bijkomt" en
de knop "Stoppen". Een andere zoekopdracht toont "Vervangen door deze", met de
controlevraag uit huisstijl §19. Het blok staat er alleen met een eigen
profiel, en niet als er een naam in het zoekveld staat (TT-257). De straal die
wordt bewaard is de ingestelde, niet de verruimde (TT-62). Instellingen →
E-mailvoorkeuren kreeg een tweede blok met de bewaarde zoekopdracht en
"Stoppen". Nieuwe tabel `musician_saved_search`; `tt_digest_new_musicians`
leest die tabel met dezelfde filters als de app, plus blokkades in beide
richtingen. De Edge Function `send-digest` verandert daarvoor niet.

**Besluit Ronald, 26-09-2026:** staat de mail op "Geen" en bewaart iemand een
zoekopdracht, dan gaat de mail op Dagelijks, met een melding erbij. Ronald:
"dan wordt de gebruiker vanzelf bewust van de instelling."

**Laag 2, 26-09-2026, op talenttent.org, ingelogd, 375px breed.** Geverifieerd:
de app laadt `core.js`, `search.js` en `styles.css` met `?v=20260926e`; het
SQL-script gaf twee keer `true`. Zoeken op Gitaar gaf nergens een treffer; de
lege staat toonde het blok. Bewaren schreef één rij in
`musician_saved_search` (Gitaar, 10 km, eigen woonplaats). Een zoekopdracht op
25 km toonde "Vervangen door deze" en de controlevraag met beide
zoekopdrachten; "Ja, vervangen" gaf één rij met 25 km. Instellingen toonde de
zoekopdracht; Stoppen haalde de rij weg en toonde "Nog geen". De RLS-regels
laten dus lezen, schrijven, bijwerken en wissen van de eigen rij toe. De
eindstand is als vooraf: geen bewaarde zoekopdracht, mail op Dagelijks.
**Niet gemeten op de echte site:** de stap "mail stond uit" (alleen laag 1,
blok 39) en een echte mail met nieuwe muzikanten — die hoort bij TT-323.

**UX-toets (senior UX-skill):** twee wijzigingen verwerkt. (1) Na automatisch
verruimen staat het blok bovenaan, direct onder de verruim-regel: juist dan zit
de juiste er dichtbij nog niet bij. (2) "Vervang je bewaarde zoekopdracht"
liep over twee regels; nu "Vervangen door deze". Oordeel: opleveren.
Schermafdrukken op 375px bekeken.

**Getoetst.** SQL twee keer gedraaid op een eigen PostgreSQL 16 met proefdata:
de filters (straal, vertrekpunt, instrument, genre, leeftijd, niveau, nieuw,
profiel af, blokkade) geven precies de verwachte muzikanten; zonder bewaarde
zoekopdracht niets; RLS laat een ander de rij niet zien en niet schrijven.
Testset: blok 39, achttien controles, zakt tegen de oude code. **567 van 567
geslaagd.**

**TT-334.** "Werkt de knop niet? Tik dan op deze link." — alleen "deze link"
is klikbaar. In de vier Edge Functions (`ouder-toestemming`, `send-digest`,
...
telkens Deploy; (4) bij "Reset Password" en "Change Email Address" de HTML
vervangen door `_niet-uploaden-TT-334-supabase-*.html`. Daarna laag 2 door
Claude, en TT-323 opnieuw voor alle mails.

**Gewijzigd:** `index.html`, `styles.css`, `core.js`, `search.js`,
`tests/tt_tests.py`, `tests/stub/supabase-stub.js`, `actielijst.md`,
`CHECKSUMS.txt`.

---

**Vorige update:** 26-09-2026 (vervolg 3) — **TT-337 is P0 (besluit Ronald)
...
steeds een nieuwe code. De oude code blijft nu geldig, in de nieuwe kolom
`ouder_toestemming.oude_code_hashes`. Alle links vervallen samen met de
aanvraag. Een link die toch niet werkt, toont nu een melding in plaats van de
homepage: "Deze link werkt niet. Heb je meer mails van ons gekregen? Gebruik
dan de knop in de nieuwste."

**TT-331.** De knop in de mail "Je hebt toestemming" gaat naar
`#toestemming-gegeven`. Staat het profiel in die browser, dan gaat het kind
gewoon verder. Anders legt de pagina uit waar het profiel staat. Ingelogd gaat
de knop naar Mijn Profiel, zoals eerst. **Bekende grens:** is in die browser
een ánder account ingelogd, dan komt het kind daar ook op Mijn Profiel uit.

**TT-332.** Beide eindschermen van de ouder, toestemming en weigering, sluiten
met "Je kunt dit venster nu sluiten."

**TT-333.** "Verzoek" is "aanvraag" in de ouderschermen en in de foutmelding van
...
**Gewijzigd:** `index.html`, `core.js`, `ouder.js`, `tests/tt_tests.py`,
`actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 26-09-2026 (vervolg) — **TT-72 deel 1 gebouwd: de
welkomstmail. Nieuw: TT-336 (advies P0).**

**Gebouwd.** De Edge Function `welkom` stuurt de welkomstmail zodra een profiel
online staat, voor 16+ én voor 13 t/m 15. De database roept haar aan via de
trigger `tt_welkom_mailen`; de app verandert niet. Wie al een profiel heeft,
krijgt geen welkomstmail achteraf. Opmaak C, TT-MAILTEMPLATE v3 ongewijzigd.

**Gedaan door Ronald, 26-09-2026:** de functie `welkom` aangemaakt en
`_niet-uploaden-TT-72-welkomstmail-26-09-2026.sql` gedraaid, beide geslaagd
(melding Ronald). Nog niet gemeten: een echte welkomstmail — hoort bij TT-323.

**Besluit Ronald, 26-09-2026:** de UX-toets kijkt voortaan altijd mee
(projectinstructies §2, regel 17).

**Nieuw:** TT-336 — het e-mailadres wordt bij registratie niet bevestigd.

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 26-09-2026 — **TT-323 gestart en door Ronald gestopt.
Vijf van de zeven mails ontvangen, zes bevindingen. Geen codewijziging.**

**Waarom gestopt.** Claude noemde de mails bij een nummer (1 t/m 7). In de
inbox staat geen nummer, dus Ronald wist niet welke mail bedoeld was. Claude
bedoelde met "de pane" het scherm rechts in de Claude-app en zei dat niet.
**Les voor de volgende ronde:** noem een mail bij zijn onderwerpregel, noem
"het scherm rechts" bij naam, en geef één stap tegelijk.

**Stand per mail** (onderwerpregel zoals in de inbox):

| Onderwerpregel | Aan | Uitslag |
|---|---|---|
| Kies een nieuw wachtwoord | talenttent@proton.me | verstuurd, nog niet beoordeeld |
| Bevestig je nieuwe e-mailadres | talenttent+wijzig@proton.me | aangekomen; lange link → TT-334 |
| Proef (14) vraagt je toestemming voor Talent Tent | talenttent+ouder@proton.me | aangekomen; knop kwam op de homepage → TT-330 |
| Proef wacht nog op je toestemming | talenttent+ouder@proton.me | aangekomen; goedkeuren werkte |
| Je hebt toestemming — maak je profiel af | talenttent+kind@proton.me | aangekomen, afzender en onderwerp kloppen; knop → TT-331 |
| digest | — | nog niet verstuurd |
| Melding: {reden} — {naam} | privacy@ | nog niet verstuurd |

**Geverifieerd (DNS, 26-09-2026):** SPF `v=spf1 a mx include:spf.mijndomeinhosting.nl
ip4:45.82.191.150 -all` — in orde. DMARC `p=none` (alleen rapporteren). DKIM
niet gevonden onder de gangbare namen; **onbekend** tot de kopregel
`Authentication-Results` van een ontvangen mail is gelezen.

**Proefgegevens die nu bestaan:**
- Het proefaccount van TT-42 (`ronaldwever+kind@proton.me`) wacht op de
  bevestiging naar `talenttent+wijzig@proton.me`. `talenttent@proton.me` zelf
  kon niet: daar bestaat al een account (Supabase: `email_exists`).
- Aanvraag `a7529ca6-d8a9-42c0-ac5b-b0b6651a4f77` (kind "proeftt323", 14 jaar)
  is goedgekeurd. `aangemaakt_op` staat 8 dagen terug, voor de herinnering.
  In het scherm rechts staat "Kies je wachtwoord"; het account bestaat pas
  daarna.

**Afspraak Ronald, 26-09-2026:** alle bevindingen uit TT-323 worden in één
ronde opgelost. Daarna worden de gewijzigde mails opnieuw ontvangen.

**Nieuw:** TT-330 en TT-331 (advies P1), TT-332, TT-333 en TT-334 (P2),
TT-335 (P3). Zie de rijen.

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 15) — **TT-325 ingevoerd door Ronald,
nagemeten in de browserpane.** Geverifieerd: de drie Edge Functions in Supabase
(`ouder-toestemming`, `send-digest`, `melding-privacy`) zijn teken voor teken
gelijk aan de bestanden `_niet-uploaden-TT-325-edge-function-*.ts` (SHA-256,
op de slotregel na); "Reset password" en "Change email address" hebben het
nieuwe onderwerp en de nieuwe HTML; `talenttent.org/mail-woordmerk.png` laadt
(440×46); beide repo's zijn gelijk. **Volgende stap:** TT-323 — van elke mail
één echte ontvangen. TT-325 is af als die slaagt.

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 14) — **TT-325 gebouwd: alle zeven
mails op TT-MAILTEMPLATE v3, met de teksten die Ronald in het Excel-bestand
vaststelde. Geen app-code gewijzigd.** Nog te doen, door Ronald: invoeren in
Supabase (zie de rij van TT-325), daarna TT-323 voor alle zeven mails.

**Gemaakt, in de gedeelde map (niet naar de repo):** drie Edge Functions
(`_niet-uploaden-TT-325-edge-function-ouder-toestemming.ts`, `-send-digest.ts`,
`-melding-privacy.ts`), twee Supabase-sjablonen
(`_niet-uploaden-TT-325-supabase-wachtwoord-vergeten.html`,
`-email-wijzigen.html`) en het voorbeeld
`_niet-uploaden-TT-325-mails-gebouwd-25-09-2026.html`.
**Geverifieerd:** het template-blok is in de drie functies byte voor byte
gelijk; de drie functies doorstaan een typecontrole (`tsc`, strict; alleen de
twee bestaande parameters van `withSupabase` hebben geen type); alle negen
voorbeelden zijn 375px breed zonder zijwaarts scrollen, zonder achtergebleven
`{invulplek}` en zonder "The Talent". **Niet getoetst:** echt versturen — dat
kan alleen in Supabase (TT-323).

**Besluiten Ronald, 25-09-2026 (vervolg 14):** de mail aan de ouder noemt geen
adres voor vragen ("ik ben alleen en heb geen tijd om vragen te
beantwoorden"); mail 5 zegt "hetzelfde toestel en dezelfde browser".
**Rechtgezet in hetzelfde gebaar:** de projectinstructies (§2.6a, §7, §9, §10,
§11) en `minderjarigen-toestemming-25-09-2026.md` (§9) zeiden dat de mail aan
de ouder eindigt met "Vragen? contact@". Beide documenten zijn vervangen, in
het project en in de gedeelde map.

**Rechtgezet:** in het Excel-bestand heette regel 4 "Regel onder elke knop".
Dat klopt niet: de regel "Werkt de knop niet? Open dan deze link:" staat onder
de knop van mail 1 tot en met 4, niet onder die van mail 5 en de digest. Zo
stond het ook in het voorbeeld dat Ronald goedkeurde.

**Gewijzigd:** `mail-woordmerk.png` (nieuw), `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 13) — **TT-325: de open punten van
het mailtemplate zijn besloten (besluiten Ronald). Ronald past de mailteksten
zelf aan in een Excel-bestand. Geen codewijziging.** Zie de rij van TT-325.

**Gemaakt, in de gedeelde map (niet naar de repo):**
`_niet-uploaden-TT-325-zo-worden-de-mails-25-09-2026.html` (alle mails op
375px, met genummerde open punten) en
`_niet-uploaden-TT-325-mailteksten-25-09-2026.xlsx` (alle teksten, kolom
"Nieuw" voor Ronald). De code staat ook in de gedeelde map, zodat de
volgende sessie niets opnieuw hoeft te maken:
`_niet-uploaden-TT-325-mailtemplate-v2.ts` (het template),
`_niet-uploaden-TT-325-mails.ts` (de zeven mails, nog met de teksten van vóór
vervolg 13) en `_niet-uploaden-TT-325-voorbeeld-maken.ts` (maakt de
voorbeeldpagina, met `node --experimental-strip-types`). **Volgende stap:**
Ronald vult het Excel-bestand in; daarna zet Claude de teksten in
`tt-mails.ts`, verwerkt besluit 7, en bouwt de drie Edge Functions en de twee
Supabase-sjablonen.
...
**Vorige update:** 25-09-2026 (vervolg 12) — **TT-327 afgehandeld: laag 2
geslaagd.** Gemeten in de browserpane op talenttent.org, ingelogd, op 375px
breed. De app laadt `core.js` en `search.js` met `?v=20260925d`. Het scherm
E-mailvoorkeuren toont alleen "Nieuwe-matches e-mail". Opslaan sluit het scherm
zonder fout, en de database geeft daarna `daily` terug. `email_theme` van het
eigen profiel is `light`: de SQL van Ronald is gelukt. **Vier P0-bouwtickets
open:** TT-295 · TT-65 · TT-325 · TT-329.

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 11) — **TT-327 gebouwd.** Het veld
"E-mailstijl" (Licht/Donker) is weg uit Instellingen → E-mailvoorkeuren, samen
met `selectEmailTheme()`, `setEmailTheme()` en `emailThemeValue`. Openen leest
`email_theme` niet meer, opslaan schrijft het niet meer. De kolom blijft in de
database. **Volgorde:** de regel "eerst de Edge Function, dan de app" vervalt.
Reden, Ronald: de mailfunctie werkt nog niet, dus niemand krijgt een mail; alle
profielen zijn testprofielen die worden verwijderd. Ronald zet iedereen op Licht
met `_niet-uploaden-TT-327-alles-licht.sql`. Nieuw: blok 38 in de testset.
Testset 540 van 540, monitorronde 21 van 21. **Nog te doen:** uploaden, SQL
draaien, laag 2 (Instellingen → E-mailvoorkeuren openen en opslaan).

**Gewijzigd:** `index.html`, `core.js`, `search.js`, `tests/tt_tests.py`,
`actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 10) — **TT-327 is P0 (besluit Ronald):
de keuze tussen een lichte en een donkere mail gaat uit Instellingen. De mails
blijven licht. TT-327 gaat mee met TT-325. Vijf P0-bouwtickets open: TT-295 ·
TT-65 · TT-325 · TT-329 · TT-327. Geen codewijziging.**

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 9) — **TT-325 en TT-329 zijn P0
(besluit Ronald): "de emails moeten goed zijn voor de livegang." Was P2 en P3.
Vier P0-bouwtickets open: TT-295 · TT-65 · TT-325 · TT-329. Geen codewijziging.**

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 8) — **TT-329 afgesplitst uit
TT-325 (besluit Ronald): de mailteksten zelf aanpassen wordt een eigen ticket.
TT-325 houdt de opmaak. Geen codewijziging.**

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 7) — **TT-328 gebouwd: het woordmerk
...
**TT-312 — band opheffen** (browserpane, ingelogd als Ronald, echte database):
- "Proefband TT-312" aangemaakt langs dezelfde weg als `saveBandRun()`: de
  band, Ronald als Oprichter, en via `tt_save_band_wanted` twee instrumenten
  bij "Gezocht".
- Opgeheven via de knoppen: Bandleden beheren → Beheer overdragen → Band
  opheffen. De geladen `bands.js` (`?v=20260925a`) roept `tt_dissolve_band`
  aan.
- **Geverifieerd:** daarna nul rijen in `bands`, `band_members` en
  `band_wanted` voor die band. Geen consolefout. Silver Earring en Van Delft
  staan er nog.
- **Niet in productie getoetst:** een gewoon lid dat probeert op te heffen.
  Dat zou bij een fout de band van een ander wissen. Lokaal getoetst op
  25-09-2026 (geweigerd, niets gewist); uitgelogd geeft productie 42501.

**TT-42 — registratie met toestemming van een ouder** (browserpane, 375px):
- Claude vulde de wizard in als "proeftt42", geboren 01-03-2012 (14 jaar),
  mailadres `ronaldwever+kind@proton.me`. Het wachtwoordveld stond er niet;
  de regel "Onder de 16 vragen we straks toestemming" wel.
- Verzoek verstuurd naar `ronaldwever+ouder@proton.me`. Aanvraag
  `7a6fd705-2355-4bb1-b16f-b08efa3704cc`, stand open, vervalt 09-10-2026.
- Ronald vinkte de voorwaarden aan, keurde goed vanuit de mail aan "+ouder",
  en koos op het wachtwoordscherm zelf het wachtwoord (Claude voert geen
  wachtwoorden in).
- **Geverifieerd:** ingelogd als `ronaldwever+kind@proton.me`; profiel
  "proeftt42" bestaat, `profile_complete` waar; de aanvraag staat op
  goedgekeurd en is uit de browser gewist.
- **Geverifieerd door Ronald met SQL:** `ouder_toestemming` voor deze
  aanvraag geeft `goedgekeurd` en `musician_id`
  `014b04a9-084f-411e-bb0f-58982666cc01` — hetzelfde id als het profiel. Het
  koppelen werkt.
- **Geverifieerd door Ronald:** beide mails kwamen aan, aan "+ouder" en aan
  "+kind".
- **Nog te doen door Ronald:** het proefaccount verwijderen (Instellingen →
  Account verwijderen) en weer inloggen met zijn eigen account.

**Nieuw:** TT-325 (P2) — tekst en opmaak van de twee mails van de
ouderroute, op verzoek van Ronald. TT-326 (P3) — één consolefout 406 tijdens
de doorloop, bron onbekend.

**Gewijzigd:** `actielijst.md`, `CHECKSUMS.txt`.

---

**Vorige update:** 25-09-2026 (vervolg 4) — **TT-304: de monitor wacht
...
menu krijgt een donkere laag erachter én een lichter menuvlak (optie B);
(2) er staat maar één menu tegelijk open; (3) goud nooit als doorschijnend
vlak, op álle plekken, ook de gouden gloeden; (4) de wizard telt met één
teller.

**Wat er gebouwd is.**
- **Menu's (TT-313, TT-314).** `core.js` kreeg `sluitAlleMenus()`,
  `menuLaagOpen()` en `menuLaagWeg()`. Elk menu roept `sluitAlleMenus()` aan
  vóór het opent: hamburger, ⋯ op het profiel, ⋯ bij een band, ⋯
  melden/blokkeren (`veiligheid.js`) en de keuzemenu's (`utils.js`). De laag
  staat direct vóór het menu, in dezelfde ouder; een tik erop sluit het menu en
  bereikt niets eronder. Een ouder met eigen z-index (kop, navigatierij) gaat
  zolang mee omhoog (`.menu-drager`), anders bleef de onderbalk erboven. Vier
  losse document-luisteraars voor klik en Escape zijn weg; één Escape-luisteraar
  sluit alles. Op desktop valt de laag binnen het canvas, net als een modal.
- **Goud (TT-290, TT-259, plus acht plekken).** Zoektabblad, focusrand,
  ongelezen gesprek, gekozen niveau, hover van kruisje/terug/ⓘ, uploadvlak,
  tipvak mediahoek, afgeronde wizardstap, de "bezig met bewerken"-balk en de
  zoektreffer in suggesties. Gloeden weg: onder de primaire knop en de
  zoekknop, achter het logo, en de gouden waas op de achtergrond
  (`body::before` — die hele vaste laag is verwijderd).
- **Bijvangst TT-259.** Een tekstveld kreeg bij focus nooit een gouden rand:
  de basisregel voor velden was specifieker dan de focusregel. Alleen de gloed
  was te zien. Rechtgezet; een veld met een fout krijgt bij focus een rode
  rand van 2px.
- **Wizard (TT-250).** "Stap 2 van 5". De aparte labels voor bewerkmodus zijn
  weg.

**Geverifieerd.** Blok 30 van de testset: 23 controles. Tegen de oude code
zakken er acht en daarna breekt het blok af (`sluitAlleMenus` bestaat niet).
...
**Bijgewerkt 22-09-2026 (vervolg).** *(Achterhaald 25-09-2026 (vervolg 5): laag 2 is gedraaid en geslaagd; TT-42 telt niet meer mee.)* TT-42 is gebouwd, maar blijft in deze stand meetellen tot laag 2 is gedraaid: de keten mail → klik van de ouder → terugkomst van het kind raakt de database en de mailserver en is niet tegen de stub te toetsen. *(Rechtgezet 23-09-2026: hier stond ook "Hij wacht bovendien op twee handelingen van Ronald (het SQL-script en de Edge Function)." Beide zijn op 22-09-2026 gemeten, de dagelijkse taak op 23-09-2026.)* *(Rechtgezet 25-09-2026: hier stond "Zolang blijven het er vijf." Het zijn er nu vier; zie de stand hieronder.)*
...
**Stand van de P0's, bijgewerkt 26-09-2026 (TT-336).** **Vier P0-bouwtickets
staan open:** TT-65 · TT-325 (gebouwd; wacht op de tekstenlijst van Ronald) ·
TT-329 · TT-336 (gebouwd; wacht op de invoer in Supabase en laag 2). TT-344
(P0, besluit Ronald) is gebouwd en wacht op laag 2. Daarnaast de controle
TT-323.

*(Achterhaald 26-09-2026 (TT-336): zie de stand hierboven.)*
**Stand van de P0's, bijgewerkt 26-09-2026 (TT-344).** **Vier P0-bouwtickets
staan open:** TT-65 · TT-325 (gebouwd; wacht op de tekstenlijst van Ronald) ·
TT-329 · TT-336. TT-344 (P0, besluit Ronald) is gebouwd en wacht op laag 2.
Daarnaast de controle TT-323.

*(Achterhaald 26-09-2026 (TT-344): zie de stand hierboven.)*
**Stand van de P0's, bijgewerkt 26-09-2026 (TT-323, vervolg).** **Vier
P0-bouwtickets staan open:** TT-65 · TT-325 (gebouwd; wacht op de
tekstenlijst van Ronald) · TT-329 · TT-336. Daarnaast TT-344 (advies P0, door
Ronald te bevestigen) en de controle TT-323. TT-295, TT-62 (deel 2) en TT-337
zijn afgehandeld; ze staan in de tweede tabel.

*(Achterhaald 26-09-2026 (TT-323, vervolg): zie de stand hierboven.)*
**Stand van de P0's, bijgewerkt 26-09-2026 (vervolg 4).** **Zeven P0-bouwtickets
staan open:** TT-295 (laag 2 geslaagd, wacht op een echte mail in TT-323) · TT-65 · TT-62 (deel 2,
gebouwd met TT-295, laag 2 geslaagd) · TT-325 · TT-329 · TT-336 · TT-337
(gebouwd, wacht op laag 2). Daarnaast de controle TT-323.
*(TT-336 is op 26-09-2026 (vervolg 4) P0 geworden, besluit Ronald. Wacht nog
steeds: "later", besluit Ronald 26-09-2026.)*
...
en getest en staat in de tweede tabel; zijn twee tabellen staan sinds
18-09-2026 in productie, alleen laag 2 staat nog open. TT-281 kwam er
...
| **TT-336** | E-mailadres wordt bij registratie niet bevestigd | **Gebouwd en getest 26-09-2026 (TT-336), zie Laatste update. Wacht op Ronald: SQL-script, Edge Functions `email-bevestigen` en `welkom`, repo-bestanden. Daarna laag 2.** **Besluiten Ronald 26-09-2026: 1a (geen band vóór de klik) en 2a (e-mailadres aanpassen op "Nog één stap").** *(Rechtgezet 26-09-2026 (vervolg 2): deze rij stond in de tabel "Afgehandeld of geblokkeerd bij Ronald"; het ticket is open.)* **Niveau P0, besluit Ronald 26-09-2026 (vervolg 4).** Nieuw, 26-09-2026, wens Ronald. Mailbevestiging staat uit in Supabase: wie zich aanmeldt, kan elk adres invullen. **Toets P0:** tikt iemand zijn eigen adres verkeerd, dan komt "Wachtwoord vergeten" nooit aan en is hij zijn account kwijt; en iemand kan zich aanmelden met het adres van een ander. **Gemist door Claude:** bij het ontwerp van de welkomstmail (TT-72) zag Claude dit en schreef er de regel "Niet zelf aangemeld?" omheen, in plaats van het als bevinding te melden. **Waarom niet de instelling in Supabase aanzetten:** de app maakt het account aan en logt meteen in (`createAccountAndProfile()` en `submitProfile()` in `wizard.js`); met bevestiging aan weigert Supabase dat inloggen en loopt elke registratie vast. Geldt bovendien voor iedereen. **Wens Ronald:** voor 16+ dezelfde route als voor 13 t/m 15 (route A, `ouder.js`, Edge Function `ouder-toestemming`), alleen gaat de mail naar de gebruiker zelf. Het account ontstaat pas na de klik. Neem TT-331 mee: de link werkt nu alleen in de browser waar iemand begon. **Open vraag aan Ronald:** bevestigen aan het begin van de wizard of aan het eind (advies Claude: aan het eind). Zodra dit werkt, gaat de regel "Niet zelf aangemeld?" uit de welkomstmail. Eigen sessie. **Ontwerp, 26-09-2026 (TT-344-sessie), nog niet gebouwd — besluiten Ronald zie Laatste update van die dag.** *(De "Wens Ronald" hierboven — het account pas na de klik, zoals route A — is daarmee vervangen: Ronald vroeg om de gangbare werkwijze.)* **Stroom voor 16+:** (1) stap 1 zoals nu: e-mail en wachtwoord, het account ontstaat daar al (`createAccountAndProfile()`, rij met `profile_complete = false`); (2) "Profiel aanmaken": het profiel blijft onzichtbaar en de app vraagt de mail aan; een scherm "Nog één stap" noemt het adres; (3) de knop in de mail opent `#bevestig/<code>`, op elk toestel en in elke browser; het profiel komt online en de welkomstmail volgt vanzelf (trigger `tt_welkom_mailen`). **Vóór de klik (besluit "a"):** rondkijken en zoeken mag; niet vindbaar, geen berichten, geen band oprichten (een band zou de oprichter alsnog vindbaar maken — besluit Ronald 26-09-2026, 1a; was advies Claude). **Database (SQL-script, Ronald draait het):** kolommen `musicians.email_bevestigd_op` en `musicians.wacht_op_bevestiging`; alle profielen die nu online staan, gelden als bevestigd. Tabel `email_bevestiging` (code als sha-256, oude codes blijven geldig zoals bij TT-330, dicht voor anon en authenticated). Trigger vóór insert/update op `musicians`: de app mag de twee nieuwe velden nooit zelf zetten; gaat `profile_complete` voor het eerst naar waar bij 16+ zonder bevestiging, dan blijft het onwaar en wordt `wacht_op_bevestiging` waar. Alleen bij de overgang onwaar→waar, zodat wie op 15 begon en later 16 wordt, niet verdwijnt. Twee triggers vóór insert op `messages` en `bands`: weigeren zolang de afzender of oprichter wacht. **Edge Function `email-bevestigen` (nieuw, Verify JWT uit):** `start` (ingelogd; leest het account uit de meegestuurde sleutel; stuurt naar het adres van het account; minstens een minuut tussen twee mails, hooguit vijf) en `bevestig` (code; weigert als het adres van het account intussen is gewijzigd; zet `email_bevestigd_op`, en `profile_complete` als hij wachtte). Mail in opmaak A met het TT-MAILTEMPLATE-blok letterlijk gekopieerd; teksten in een TEKSTEN-blok voor TT-329. **`welkom`:** de regel "Niet zelf aangemeld?" alleen nog zonder bevestiging, dus bij 13 t/m 15. **App:** `saveEditedProfile()` herkent de afronding van een nieuwe registratie (`state.onboarding`), wist de voortgang en toont "Nog één stap"; een melding op Mijn Profiel ("Je profiel is nog niet zichtbaar", met "Mail opnieuw sturen"); `#bevestig/<code>` in `appInit()` en in `opstartHerstelt`, met dezelfde paginaopbouw als de goedkeuringspagina; `insertMessage()` en het oprichten van een band vragen eerst `emailWachtOpBevestiging()`. **Onderweg gezien (Aanname, nog te meten):** bij 16+ eindigt "Profiel aanmaken" nu in `saveEditedProfile()`, met de melding "Profiel bijgewerkt!" in plaats van "Profiel aangemaakt!", en de bewaarde voortgang wordt niet gewist — dan blijft "Je bent nog bezig met je profiel" op Mijn Profiel staan. Hoort bij dezelfde bouw. **Volgorde bij invoeren:** SQL → functies → repo-bestanden. **Laag 2:** een nieuw 16+-proefaccount; vóór de klik niet vindbaar en geen bericht mogelijk; na de klik vindbaar, welkomstmail zonder "Niet zelf aangemeld?". |
| **TT-344** | "Wachtwoord vergeten" komt uit op Mijn Profiel | **Gebouwd en getest 26-09-2026 (TT-344), zie Laatste update. Wacht op laag 2. Niveau P0, besluit Ronald 26-09-2026.** Nieuw, 26-09-2026, TT-323. *(Was: advies P0.)* **Toets P0:** wie zijn wachtwoord kwijt is, kan geen nieuw kiezen en raakt zijn account kwijt. In Instellingen vraagt "Wachtwoord wijzigen" om het huidige wachtwoord. **Geverifieerd op talenttent.org:** een verse link uit "Kies een nieuw wachtwoord", uitgelogd geopend, eindigt na vier seconden op Mijn Profiel, ingelogd. **Oorzaak (Aanname, uit de code en de bron van supabase-js):** de link logt in. supabase-js meldt `PASSWORD_RECOVERY` pas na `setTimeout(0)`. `appInit()` in `core.js` heeft dan al `onUserLoggedIn()` aangeroepen. Die wacht op de database en roept daarna `showView('myprofile')` aan, omdat `#access_token=…` niet meetelt in `opstartHerstelt`. Zo overschrijft Mijn Profiel het scherm "nieuw wachtwoord" — hetzelfde patroon als TT-337. **Voorstel:** een link met `type=recovery` laten meetellen in `opstartHerstelt`, en in `onUserLoggedIn()` niet doorsturen zolang de view `reset` is. Eerst een controle in de testset die zakt tegen de huidige code. Hangt samen met TT-346 (zelfde route). |
...
| ID | Ticket | Kern |
|---|---|---|
| **TT-295** | De matchhelft van de digest levert structureel niets op | **Afgehandeld 26-09-2026 (TT-323, vervolg): echte mail aangekomen — zie Laatste update.** **Gebouwd en getest 26-09-2026 (vervolg 4): de bewaarde zoekopdracht. SQL gedraaid, laag 2 geslaagd. Wacht nog op één echte mail met nieuwe muzikanten (TT-323) — zie Laatste update.** **Nieuw, 20-09-2026.** `tt_digest_new_musicians` doet een inner join op `musician_wanted`. Die tabel bevat **0 rijen**, geverifieerd 20-09-2026 tegen productie. TT-232 (09-09-2026) haalde het enige invulveld ervoor — "instrumenten die je zoekt in een ander" — uit Zoekvoorkeuren, met als reden "dat staat al in de zoekfilters". Sindsdien kan niemand die tabel nog vullen, en dus vindt de nachtelijke query per definitie niemand. De bandhelft werkt wel: die gebruikt `band_wanted` tegen de instrumenten in je eigen profiel, en is op 20-09-2026 aantoonbaar in een echte mail terechtgekomen (drie bands). **Toets:** kan de app hiermee live zonder dat een gebruiker iets misloopt? Nee — de app belooft een mail over nieuwe matches en levert die helft niet. Zelfde grond waarop TT-01 P0 was. **Voorstel Ronald, geen besluit (20-09-2026):** een bewaarde zoekopdracht — een vinkje op het zoekformulier dat de héle zoekopdracht opslaat (instrument, genre, straal, plaats), niet alleen een lijstje instrumenten. Dat zou TT-295 en TT-62 deel 2 in één keer afhandelen. Alternatief: het oude veld terugzetten in E-mailvoorkeuren — kleiner werk, armere mail, TT-62 deel 2 blijft open. Nog geen ontwerpsessie |
| **TT-62 (deel 2)** | "Geef me een seintje zodra er een drummer bijkomt" | **Afgehandeld 26-09-2026 (TT-323, vervolg): echte mail aangekomen — zie Laatste update.** **Gebouwd 26-09-2026 (vervolg 4), samen met TT-295: "Mail me als er iemand bijkomt" staat onder elk resultaat op het tabblad Muzikant, en bovenaan na verruimen. Laag 2 geslaagd, zie Laatste update.** **Uitzondering vastgelegd 12-09-2026 (besluit Ronald, TT-257):** het automatisch verruimen van deel 1 geldt **niet** als er een naam in het zoekveld staat. Wie op naam zoekt, zoekt één bepaalde persoon; iemand twee provincies verderop is dan geen beter antwoord dan geen antwoord. Zie huisstijl §17. **Deel 1 is gebouwd 11-09-2026** (automatisch verruimen, zie Deel 3). Deel 2 maakt van een dood einde een afspraak die het systeem bewaakt in plaats van de gebruiker. Leunt op dezelfde verzendweg als TT-01 en kan dus niet eerder |
| **TT-337** | Een ingelogde ouder komt op zijn eigen profiel uit in plaats van op de goedkeuringspagina | **Afgehandeld 26-09-2026 (TT-323, vervolg): laag 2 geslaagd — zie Laatste update.** **Gebouwd en getest 26-09-2026 (vervolg 3), zie Laatste update bovenaan. Wacht op laag 2.** **Niveau gewijzigd 26-09-2026, besluit Ronald: P0.** Was advies P1. **Toets P0:** een ingelogde ouder kan niet goedkeuren; het kind komt er niet in. Nieuw 26-09-2026 (vervolg 2), laag 2 van TT-330. Geverifieerd op talenttent.org: met een ingelogd account toont `#toestemming/<code>` eerst de goedkeuringspagina, daarna springt de app naar Mijn Profiel. Oorzaak: `onUserLoggedIn()` in `core.js` gaat na het laden naar Mijn Profiel, omdat `#toestemming/` niet meetelde in `opstartHerstelt`. Bestond al sinds TT-42. **Gebouwd:** `#toestemming/` telt mee in `opstartHerstelt`; op de goedkeuringspagina verschijnt het gebruikersnaamscherm (TT-38) niet. Blok 28: drie controles, zakken tegen de oude code. |
| **TT-312** | Band opheffen wist leden zonder foutcontrole | **Afgehandeld 25-09-2026 (vervolg 5): gebouwd, SQL-script gedraaid, laag 2 geslaagd — zie Laatste update.** *(Rechtgezet 25-09-2026: hier stond "wacht op het SQL-script van Ronald en laag 2".)* Nieuw, 23-09-2026. `dissolveBand()` (`bands.js`) wiste "Gezocht" en de leden zonder controle, daarna pas de band. Mislukte dat laatste, dan bleef een band zonder leden en oprichter over. Besluit Ronald 25-09-2026: één transactie, als TT-281 (`tt_dissolve_band`) |
| **TT-42** | Registratie en toestemming voor 13-15-jarigen | **Afgehandeld 25-09-2026 (vervolg 5): laag 2 geslaagd — zie Laatste update.** Gebouwd 22-09-2026, route A *(rechtgezet 23-09-2026: hier stond "wacht op twee handelingen van Ronald"; beide zijn op 22-09-2026 gemeten)*. In productie en geverifieerd: de tabel `ouder_toestemming`, de drie functies, de Edge Function `ouder-toestemming` (Verify JWT uit), en de dagelijkse taak `tt-ouder-onderhoud` (23-09-2026, uitslag van Ronald). Zeven onderdelen in `ouder.js` en `view-toestemming`. **Laag 2:** de keten mail aan de ouder → klik → mail aan het kind → wachtwoord, op de echte site, met een mailadres waar Ronald bij kan. Ontwerp en besluiten: `_niet-uploaden-tt42-ontwerp-22-09-2026.md` en `minderjarigen-toestemming-23-09-2026.md` |
| **TT-327** | Keuze Licht/Donker bij E-mailvoorkeuren weghalen | **Afgehandeld 25-09-2026: gebouwd, SQL gedraaid, laag 2 geslaagd — zie Laatste update.** **Nieuw, 25-09-2026 (vervolg 6), volgt uit besluit Ronald bij TT-325:** elke mail is licht. Zodra `send-digest` op het template draait, doet de keuze in Instellingen → E-mailvoorkeuren niets meer. Een knop die niets doet, is fout. Weg: `#emailThemeControl` in `index.html`, `selectEmailTheme()`, `setEmailTheme()` en `emailThemeValue` in `core.js`, en `email_theme` uit het opslaan. **De kolom `musicians.email_theme` blijft in de database staan** (voorstel van Claude): dat kost niets, en komt er ooit toch een donkere versie, dan staat ieders oude keuze er nog. ~~**Volgorde is bindend:** eerst de Edge Function, dan de app.~~ *Vervallen 25-09-2026 (vervolg 11): de mailfunctie werkt nog niet en alle profielen zijn testprofielen (Ronald). Iedereen gaat op Licht met `_niet-uploaden-TT-327-alles-licht.sql`.* **Besluit Ronald, 25-09-2026 (vervolg 10):** "de emails blijven voorlopig licht. misschien wel voor altijd" en "de keuze tussen lichte en donkere email kan er inderdaad uit". **Niveau gewijzigd naar P0**, als antwoord op de vraag of TT-327 met TT-325 meegaat; was P2. **Toets P0:** bij livegang mag er geen keuze in de app staan die niets doet. ~~**Gaat mee met TT-325**, in dezelfde oplevering, na de Edge Function~~ **Gebouwd 25-09-2026 (vervolg 11), vóór TT-325 (verzoek Ronald).** Blok 38 van de testset toetst het. |
...
| **TT-345** | Drie mails landen bij Proton in de map Sociaal | **Nieuw, 26-09-2026, TT-323. Advies P1, niveau door Ronald te bevestigen.** De aanvraag aan de ouder, de herinnering en de digest stonden in Sociaal; de andere vijf mails in de inbox (melding Ronald). DKIM, SPF en DMARC slagen, dus de authenticatie is niet de oorzaak. **Onbekend:** waarop Proton sorteert, en wat Gmail en Outlook met dezelfde mails doen. **Toets P1:** een ouder die de mail niet ziet, geeft geen toestemming; dan komt het kind niet binnen. |
...
| **TT-332** | Na de toestemming van de ouder ontbreekt een afsluiter | **Afgehandeld 26-09-2026 (TT-323, vervolg): afsluiter gezien — zie Laatste update.** **Gebouwd en getest 26-09-2026 (vervolg 2), akkoord Ronald — zie Laatste update.** Nieuw, 26-09-2026, wens Ronald (TT-323). Na "Toestemming geven" volgt een bedankscherm zonder einde. Ronald: er moet een afsluiter komen "in de zin van: u kunt dit scherm nu afsluiten". **Toets P2:** het werkt, maar de ouder weet niet dat hij klaar is. **Voorstel voor de tekst:** "Je kunt dit venster nu sluiten." — de pagina spreekt de ouder met "je" aan. |
| **TT-333** | "Verzoek" in de app, "aanvraag" in de mails | **Afgehandeld 26-09-2026 (TT-323, vervolg): "aanvraag" in de app gezien — zie Laatste update.** **Gebouwd en getest 26-09-2026 (vervolg 2), wens Ronald: ook in de app "aanvraag" — zie Laatste update.** Nieuw, 26-09-2026, TT-323. Besluit TT-325: in de mails altijd "aanvraag", nooit "verzoek". De app zegt "Je verzoek is verstuurd", "Verzoek versturen…" en op de goedkeuringspagina "Dit verzoek is afgewezen" / "Een verzoek vervalt". **Toets P2:** het werkt, maar dezelfde handeling heeft twee namen. **Open vraag aan Ronald:** geldt "aanvraag" ook in de app? |
| **TT-334** | De mails tonen de volledige URL onder de knop | **Afgehandeld 26-09-2026 (TT-323, vervolg): "deze link" in de ontvangen mails — zie Laatste update.** **Gebouwd en ingevoerd 26-09-2026 (vervolg 4), akkoord Ronald op het voorstel hieronder; in Supabase nagemeten. Wacht op TT-323.** **Nieuw, 26-09-2026, feedback Ronald (TT-323):** "ik wil niet de hele url tonen. het is te lang en rommelig." Het blok "Werkt de knop niet? Open dan deze link:" staat in het template, dus in alle zeven mails: de drie Edge Functions en de twee Supabase-sjablonen. **Toets P2:** het werkt, maar oogt rommelig. **Voorstel:** "Werkt de knop niet? Tik dan op deze link." met alleen "deze link" klikbaar. De tekstversie houdt de volledige URL. |
| **TT-350** | Losse "adres" in de teksten, waar "e-mailadres" bedoeld is | **Nieuw, 26-09-2026 (TT-336), woordkeus Ronald: "noem het dan e-mailadres. hier is het ook misgegaan in de emailteksten. jij gebruikt een verkeerde term." Advies P2, niveau door Ronald te bevestigen.** **Toets P2:** alles werkt, maar "adres" kan ook een woonadres of een webadres zijn; de lezer moet raden. **Geverifieerd** (grep op de teksten in de repo, 26-09-2026): `auth.js` — "Dit is het adres dat je nu al gebruikt", "…en je oude adres werkt dan niet meer" (zie ook TT-347), "…dan is je adres gewijzigd"; `ouder.js` — "Dit adres klopt niet. Controleer het nog een keer", "Klopt het adres niet? Wijzig het dan." Daarnaast "Vul eerst het adres van de link in." in `wizard.js` en `musicians.js`: daar is een webadres bedoeld, dus hetzelfde woord met een andere betekenis (projectinstructies §6.5). **Onbekend:** de teksten van de mails in de Edge Functions en de twee Supabase-sjablonen zijn niet nagelopen. **Voorstel:** overal "e-mailadres"; bij een link "de link". Eigen sessie. De TT-336-teksten zeggen al "e-mailadres" (blok 43 toetst dat). |
| **TT-346** | Bevestigingslinks lopen via supabase.co, met een lange code | **Nieuw, 26-09-2026, TT-323, vraag Ronald: "dit kan afschrikken. kan dit niet anders?" Advies P2, niveau door Ronald te bevestigen.** De knoppen in "Bevestig je nieuwe e-mailadres" en "Kies een nieuw wachtwoord" gaan naar `fqtgilwfestzofunupnu.supabase.co/auth/v1/verify?token=…`. Het venster "Link bevestigen" is van Proton zelf; het toont elke link voluit en is vanuit de mail niet uit te zetten. **Voorstel:** de twee Supabase-sjablonen laten linken naar talenttent.org met `{{ .TokenHash }}`; de app bevestigt dan zelf met `verifyOtp()`. Het adres begint dan met talenttent.org; een code blijft erin. **Toets P2:** het werkt, maar kost vertrouwen. Hangt samen met TT-344. |
| **TT-347** | De controlevraag bij "E-mailadres wijzigen" zegt iets onjuists | **Nieuw, 26-09-2026, TT-323. Advies P2, niveau door Ronald te bevestigen.** Geverifieerd op talenttent.org: vóór het versturen vraagt de app "Je logt vanaf nu in met {nieuw adres}, en je oude adres werkt dan niet meer. Kun je bij die mailbox?" Dat klopt niet: tot de bevestiging werkt het oude adres. De mail en de melding erna zeggen dat wel goed ("Tot die tijd log je in met …"). **Toets P2:** het werkt, maar de app zegt twee verschillende dingen. **Voorstel:** "We sturen een mail naar {nieuw adres}. Pas als je daarin op de knop tikt, log je met dat adres in. Kun je bij die mailbox?" |
...
| **TT-339** | Een bewaarde zoekopdracht op het tabblad Setlist | **Nieuw, 26-09-2026, besluit Ronald: "voor later".** TT-295 werkt alleen op het tabblad Muzikant. Voor Setlist moet de bewaarde zoekopdracht ook de nummers onthouden, en de mail moet zoeken op muzikanten die die nummers spelen: een tweede databasefunctie en een aanpassing van `send-digest`. **Toets P3:** geen aanwijsbaar gevolg voor een gebruiker nu. |
| **TT-326** | Eén consolefout 406 tijdens de ouderroute | **Nieuw, 25-09-2026 (vervolg 5), laag 2 van TT-42.** Na de doorloop stond in de console "Failed to load resource: 406". Welke aanvraag het was, is **onbekend**: de browserpane toonde geen netwerkverkeer. **Aanname:** een `.single()` op een tabel zonder rij, vóór het profiel bestond. Er ging niets zichtbaar mis; het profiel is compleet. **Toets P3:** geen aanwijsbaar gevolg voor een gebruiker. Nameten bij de volgende doorloop van de ouderroute |
