...
**Laatste update:** 11-09-2026 (vervolg 7) — **TT-256: het draaiwiel opnieuw ingedeeld en het schokkerige scrollen aangepakt, app-breed.** Aanleiding: Ronald over het straalwiel — "waarom is het veld voor dit wiel beeldvullend? de cijfers moeten in ieder geval in het midden. het scrollen gaat ook niet vloeiend. dit geldt voor al deze velden. de horizontale balk heeft een lelijke kleur." Daarna: "maak het scrollen vloeiend. dit gaat door hele app schokkerig."
...
**Nog te doen, buiten deze sessie:** `huisstijl-en-consistentie.md` §7.1 spreekt de code nu op drie punten tegen — "een bladwijzer toont één wiel op volle schermbreedte", de maat "markeringsbalk: goud op 10%" en "vastklikken met `scroll-snap-type: y mandatory`" zonder de muis-uitzondering. Dat document hoort in één keer geleverd te worden (voorkeur Ronald), niet als los fragment, en verdient een eigen sessie. Ook nog open: `transition: all` staat op circa tien plekken in `styles.css` en animeert daarmee ook eigenschappen die de indeling herberekenen — apart uit te zoeken, niet halfslachtig.
